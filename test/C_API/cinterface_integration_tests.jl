...
function _evaluate_matsubara_basis_functions(uhat, matsubara_indices)
    ...
    status[] = SparseIR.spir_funcs_batch_eval_matsu(uhat, order, length(matsubara_indices), freq_indices, uhat_eval_mat)
    @test status[] == SparseIR.SPIR_COMPUTATION_SUCCESS
...
function _matsubara_sampling_evaluate(sampling, order, ndim, dims, target_dim, gIR::AbstractArray{<:Real}, giw::AbstractArray{<:Complex})
    status = SparseIR.spir_sampling_eval_dz(sampling, order, ndim, dims, target_dim, gIR, giw)
    @test status == SparseIR.SPIR_COMPUTATION_SUCCESS
    return status
end
...
function _matsubara_sampling_evaluate(sampling, order, ndim, dims, target_dim, gIR::AbstractArray{<:Complex}, giw::AbstractArray{<:Complex})
    status = SparseIR.spir_sampling_eval_zz(sampling, order, ndim, dims, target_dim, gIR, giw)
    @test status == SparseIR.SPIR_COMPUTATION_SUCCESS
    return status
end
...
function integration_test(::Type{T}, beta, wmax, epsilon, extra_dims, target_dim, order, tol, positive_only) where T
    ...
    tau_sampling = SparseIR.spir_tau_sampling_new(basis, num_tau_points, tau_points_org, tau_sampling_status)
    ...
    matsubara_sampling = SparseIR.spir_matsu_sampling_new(basis, positive_only, num_matsubara_points_org, matsubara_points_org, matsubara_sampling_status)
    ...
    tau_sampling_dlr = SparseIR.spir_tau_sampling_new(dlr, num_tau_points, tau_points_org, tau_sampling_dlr_status)
    ...
    matsubara_sampling_dlr = SparseIR.spir_matsu_sampling_new(dlr, positive_only, num_matsubara_points_org, matsubara_points_org, matsubara_sampling_dlr_status)
