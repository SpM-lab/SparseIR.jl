@testitem "basis_set.jl" tags=[:julia, :spir] begin
    ...
    @testset "consistency2" begin
        ...
        # The set holds the same samplings: same points, same results.
        gl = collect(range(1.0, 0.1; length=length(basis_f)))
        @test evaluate(bs.smpl_tau_f, gl) == evaluate(smpl_τ_f, gl)
        @test evaluate(bs.smpl_tau_b, gl) == evaluate(smpl_τ_b, gl)
        @test evaluate(bs.smpl_wn_f, gl) == evaluate(smpl_wn_f, gl)
        @test evaluate(bs.smpl_wn_b, gl) == evaluate(smpl_wn_b, gl)
    end
    ...
    @testset "unit tests LogisticKernel" begin
        ...
        # The fermionic basis rejects the regularized Bose kernel.
        @test_throws ArgumentError SparseIR.finite_temp_bases(0.1, 0.2, 1e-3;
            kernel=RegularizedBoseKernel(0.1 * 0.2))
    end
end # testitem
