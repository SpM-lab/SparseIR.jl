...
@testitem "smooth" setup=[SVETestsFunctions, CommonTestData] begin
    for Λ in (10, 42, 10_000)
        basis = FiniteTempBasis{Fermionic}(1, Λ; sve_result=CommonTestData.sve_logistic[Λ])
        check_smooth(basis.u, basis.s, 2 * maximum(basis.u(1)), 24)
        check_smooth(basis.v, basis.s, 50, 200)
    end
end

@testitem "num roots u" setup=[CommonTestData] begin
    for Λ in (10, 42, 10_000)
        basis = FiniteTempBasis{Fermionic}(1, Λ; sve_result=CommonTestData.sve_logistic[Λ])
        for ui in basis.u
...
@testitem "num roots û" setup=[CommonTestData] begin
...
@testitem "accuracy" setup=[SVETestsFunctions, CommonTestData] begin
    for stat in (Fermionic(), Bosonic()),
        Λ in (10, 42, 10_000)

        basis = FiniteTempBasis(stat, 4, Λ; sve_result=CommonTestData.sve_logistic[Λ])
...
@testitem "choose_accuracy" begin
    using Logging: with_logger, NullLogger

    with_logger(NullLogger()) do # suppress output of warnings
        @test SparseIR.choose_accuracy(nothing, nothing) ==
              (2.2204460492503131e-16, SparseIR.Float64x2, :default)
        @test SparseIR.choose_accuracy(nothing, Float64) ==
              (1.4901161193847656e-8, Float64, :default)
        @test SparseIR.choose_accuracy(nothing, SparseIR.Float64x2) ==
              (2.2204460492503131e-16, SparseIR.Float64x2, :default)
        @test SparseIR.choose_accuracy(1e-6, nothing) == (1.0e-6, Float64, :default)
        @test SparseIR.choose_accuracy(1e-8, nothing) ==
              (1.0e-8, SparseIR.Float64x2, :default)

        @test SparseIR.choose_accuracy(1e-20, nothing) ==
              (1.0e-20, SparseIR.Float64x2, :default)
        @test_logs (:warn,
