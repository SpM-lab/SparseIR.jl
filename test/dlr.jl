@testitem "Compression" setup=[CommonTestData] begin
    using Random
    ...
    for stat in (Fermionic(), Bosonic())
        ...
        basis = FiniteTempBasis(
            stat, β, ωmax, ε; sve_result=CommonTestData.sve_logistic[β * ωmax])
...
@testitem "Boson" setup=[CommonTestData] begin
    β = 2
    ωmax = 21
    ε = 1e-7
    basis_b = FiniteTempBasis{Bosonic}(
        β, ωmax, ε; sve_result=CommonTestData.sve_logistic[β * ωmax])
...
@testitem "DiscreteLehmannRepresentation" setup=[CommonTestData] begin
    for stat in (Fermionic(), Bosonic())
        β = 10_000
        ωmax = 1
        ε = 1e-12
