struct MatsubaraPoles{S<:Statistics}
    ...
    MatsubaraPoles{S}(β::Real, poles::Vector{<:Real}) where {S<:Statistics} =
        new{S}(β, poles)
end

MatsubaraPoles(stats::Statistics, β::Real, poles::Vector{<:Real}) =
    MatsubaraPoles{typeof(stats)}(β, poles)

(mp::MatsubaraPoles{Fermionic})(n::FermionicFreq) = 
    1 ./ (valueim(n, mp.β) .- mp.poles)
(mp::MatsubaraPoles{Bosonic})(n::BosonicFreq) = 
...
struct TauPoles{S<:Statistics}
    ...
    poles :: Vector{Float64}
    ωmax  :: Float64
    TauPoles{S}(β::Real, poles::Vector{<:Real}) where {S<:Statistics} =
        new{S}(β, poles, maximum(abs, poles))
end

TauPoles(stats::Statistics, β::Real, poles::Vector{<:Real}) =
...
"""
struct DiscreteLehmannRepresentation{S<:Statistics,B<:AbstractBasis{S},T<:AbstractFloat,FMAT<:SVD} <: AbstractBasis{S}
    basis  :: B
    poles  :: Vector{T}
    u      :: TauPoles{S}
...
default_matsubara_sampling_points(dlr::DiscreteLehmannRepresentation; kwargs...) =
